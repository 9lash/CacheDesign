...
class BaseCache{
	...
	// Variables which say the length of the tag, offset and index bits   
	   double tag_Size;
	   double offset_Size;
	   double index_Size;
		
	// Variables that hold the value of tag, offset and index	
		uint32_t tag;
		uint32_t offset;
		uint32_t index;
		
	// Mask variables to extract the contents from the given address 	
	...
	// Methods to extract the offset, index and tag bits from the given address
	uint32_t extract_offset(uint32_t addr);
	uint32_t extract_index(uint32_t addr);
